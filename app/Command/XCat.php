...
class XCat
{
	...
	{
		$this->initQQWry();
		echo "Add admin account/创建管理员帐号.....";
		// ask for input
		fwrite(STDOUT, "Enter your email/输入邮箱: ");
		// get input
		...
		fwrite(STDOUT, "Press [Y] to create a admin..... 按下[Y]来创建管理员账户..... ");
		$y = trim(fgets(STDIN));
		if (strtolower($y) == "y") {
			echo "Start to create a admin account";
			// create admin user
			// do reg user
			$user = new User();
